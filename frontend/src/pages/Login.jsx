    ...
    >
      ...
      >
        ...
        <div className="mb-6 text-center">
          <h2 className="text-xl md:text-2xl font-bold text-[#0F766E]">{t('login')}</h2>
          <p className="text-slate-500 text-sm font-medium mt-1">{t('loginSub', 'Please enter your credentials to access the portal')}</p>
        </div>

        {/* Login Form */}
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="relative group">
            <label className="block text-xs font-bold text-slate-400 uppercase mb-2 ml-1 tracking-[0.12em] transition-colors group-focus-within:text-[#0F766E]">
              {t('email')}
            </label>
            <input 
              required
              type="email" 
              className="w-full px-5 py-4 bg-[#F8FAFC] border border-slate-200/80 rounded-xl outline-none focus:border-[#0F766E] focus:ring-4 focus:ring-[#0F766E]/12 transition-all font-medium text-slate-800 placeholder:text-slate-300"
          ...
          <div className="flex items-center justify-between text-xs sm:text-sm font-semibold pt-1">
            <label className="flex items-center gap-2 cursor-pointer text-slate-500 select-none hover:text-slate-700 transition-colors">
              <input 
                type="checkbox" 
                className="w-4 h-4 rounded border-slate-300 text-[#0F766E] focus:ring-[#0F766E]/20" 
              />
              <span>{t('rememberMe', 'Remember me')}</span>
            </label>
            <a href="#" className="text-[#0F766E] hover:text-[#0b5c56] transition-colors hover:underline">
          ...
          <button 
            type="submit" 
            disabled={loading}
            className={`w-full relative overflow-hidden group min-h-[52px] px-6 py-3.5 font-bold text-sm sm:text-base flex items-center justify-center gap-2 rounded-xl text-white select-none transition-all duration-200 active:scale-98 mt-6 ${
