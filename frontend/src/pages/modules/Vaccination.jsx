...
function getStatusStyle(patient, t) {
...
export default function Vaccination({ t: propT }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <header className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-black text-slate-800">{t('childVax', 'Child Vaccination')}</h2>
          <p className="text-sm text-slate-400 font-medium">{t('vaccinationSub', 'Tracking Immunization Schedule (Age ≤ 5)')}</p>
        </div>
        <button
          onClick={fetchPatients}
          aria-label={t('refreshVaccinationList', 'Refresh vaccination list')}
      ...
      {loading ? (
        <div className="py-20 text-center animate-pulse text-slate-400 font-bold uppercase tracking-widest text-xs">
          {t('loadingVaccinationRecords', 'Loading Vaccination Records…')}
        </div>
      ) : error ? (
        ...
        <div className="p-10 text-center bg-white border border-slate-100 rounded-[2.5rem]">
          ...
          <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px]">
            {t('noChildrenUnder5Registered', 'No children under 5 registered')}
          </p>
        </div>
      ) : (
        <div className="space-y-4">
          {patients.map((p, i) => {
            const style = getStatusStyle(p, t);
            return (
              <motion.div
                key={p.id}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">
                        {t('ageLabelColon', 'Age:')} {p.age != null ? t('ageYearsVal', '{{count}} yr', { count: p.age }) : t('unknown', 'Unknown')}
                        {p.gender ? ` · ${t(p.gender.toLowerCase(), p.gender)}` : ''}
                      </p>
                    </div>
                  </div>
